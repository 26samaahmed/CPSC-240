...
int main()
{
    __asm {
        ...
        mov ax, 0xBEEF;             // ax = 1011111011101111
        and ax, 0000111100000000b;  // Binary Number for E which is 14
        shr ax, 8;                  // ax = 0000000000001111
        mov thirdChar, ax;          // thirdChar = 0000000000001111

        mov ax, 0xBEEF;             // ax = 1011111011101111
        and ax, 1111000000000000b;  // Binary Number for B which is 11
        shr ax, 12;                 // ax = 0000000000001011
        mov fourthChar, ax;
    ...
    Valid:
        ...
        and ax, 0000000000001111b;  // Binary Number for E which is 14
        mov firstChar, ax;          // firstChar = ax which is E

        mov ax, 0xFADE;             // ax = 1111101011011110
        and ax, 0000000011110000b;  // Binary Number for D which is 13
        shr ax, 4;                  // ax = 0000000000001101
        mov secondChar, ax;         // secondChar = ax which is D

        mov ax, 0xFADE;             // ax = 1111101011011110
        and ax, 0000111100000000b;  // Binary Number for A which is 10
        shr ax, 8;                  // ax = 0000000000001010
        mov thirdChar, ax;          // thirdtChar = ax which is A
        
        mov ax, 0xFADE;             // ax = 1111101011011110
        and ax, 1111000000000000b;  // Binary Number for F which is 15
        shr ax, 12;                 // ax = 0000000000001111
    ...
    Valid2:
        ...
        mov ax, 0xCABE;             // ax = 1100101010111110
        and ax, 0000000011110000b;  // Binary Number for B which is 11
        ...
        mov ax, 0xCABE;             // ax = 1100101010111110
        and ax, 1111000000000000b;  // Binary Number for C which is 12
